...
**Phase 0 is complete.** The repo is scaffolded with `package.json`, `tsconfig.json` (strict, ES2023, bundler resolution), `vitest.config.ts`, `.eslintrc.cjs` (with the `no-restricted-imports` rule for `@octokit/rest` outside `src/github/**`), `.env.example`, and `.gitignore` updated for `.patchwork/`. Shared domain types live in `src/types.ts`; the YAML config schema, defaults, and loader live under `src/config/`. Sample configs are in `config/targets.yaml` and `examples/{minimal,unsloth}.yaml`; the agent skill is at `.cursor/skills/oss-contributor.md`. Phase 1 (GitHub triage pipeline) has not started — there is no `src/github/`, `src/agent/`, `src/review/`, `src/reporter/`, or `src/cli/` yet.
